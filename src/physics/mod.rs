...
impl World
{
    pub fn default() -> Self
    {
        World { gravity: 9.81, gravity_direction: Vec2d::new(0, -1.0) }
    }

    pub fn tick(&self, time_in_ms: f32, tick_resolution_in_ms: f32, entities: &mut Vec<Entity>)
    {
        let num_ticks = time_in_ms / tick_resolution_in_ms;
        if (num_ticks < 1.0)
        {
            num_ticks = 1.0;
        }
        ...
        for i in 0..num_ticks
        {
            for e in entities.iter_mut()
            {
                ...
                let gravity_fragment = self.gravity_direction.clone() * (self.gravity / sim_time_in_seconds);
